...
If the project has an official Atmosphere account, then its Bluesky account can provide the official project news, while all three custom feeds can be high-value community conversations. I describe these in detail in [[How newsrooms, scientific institutions & governments can best use Bluesky]]), but the key idea is that a well-designed Custom Feed is not a broadcast channel. It is a community space.
...
This pattern has several benefits that compound over time:
...
Note: while Starter Packs can only host three custom feeds, a project can have as many as it likes.
...
On the Atmosphere, a cluster can be managed by a single Cluster Account Manager, creating economies of scale: no need for each project to have its own official account, as the cluster manager handles all starter packs and feeds across all projects.

Moreover, s/he adds **cluster custom feeds** on top. These operate across all projects in the cluster, surfacing their *best* content for audiences — policymakers, non-scientific stakeholders, interested public — who want outcomes, not day-to-day project updates. As long as the researchers use Bluesky and remember a hashtag, everything happens automatically.

The incentive for researchers is real: post about your project, get enough reactions to be picked up in the cluster feed, and reach an audience you'd never reach otherwise. And, as set out below, researchers can be connected via another Atmosphere app (Sifa) to their organisation, so those benefits transfer to the researchers' organisation as well.

### Labels

A single labeller can serve many purposes across the research community. Of course, the only people who see the labels are those who choose to. But if you're a researcher in one project, it is useful to see that the researcher you're replying to on Bluesky is actually involved in another project in the same Cluster. 

However, it is difficult to see each individual project setting up a labeler right now. Given that each researcher is in their project's Bluesky list (used for the custom feeds), something at least semi-automated should be possible. When that does become possible, then Cluster and Global labels should also become possible: imagine every scientist working on EU-financed environmental research being labelled *"EU-funded"* and "*Environment science*" automatically, making the whole community more discoverable, filterable and legible.
...
> discovering people through knowledge, and discovering knowledge through people

This is vital, because communities are all about discovering people through knowledge, and discovering knowledge through people. Those connections happen through community profiles - if you see something from somebody on any Atmosphere app that you find interesting, you can visit their Sifa profile and discover *everything* they find interesting. 
...
While these databases can be localised to the project websites, there's nothing structurally different between doing this for one project, a cluster of projects, or entire research programmes. The tools just need to know the IDs of the relevant project and community lists.
...
Researchers don't need to do anything to provide their knowledge - by using the project-level tools set out earlier, their work can be picked up and used by these communications  specialists. But it does make them and their work more directly accessible to everyone, which cannot be a bad thing.
